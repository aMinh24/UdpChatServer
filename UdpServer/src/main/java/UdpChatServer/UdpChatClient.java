    ...
    private void processServerAction(String jsonString) {
        ...
        try {
            ...
            switch (action) {
                 ...
                 case Constants.ACTION_ROOMS_LIST: if (Constants.STATUS_SUCCESS.equals(status)) { JsonObject d=responseJson.getAsJsonObject(Constants.KEY_DATA); JsonArray r=d.getAsJsonArray("rooms"); System.out.println("\nYour rooms:"); if(r.size()==0)System.out.println("None."); else for(int i=0;i<r.size();i++){JsonObject roomInfo=r.get(i).getAsJsonObject();String roomId=roomInfo.get("id").getAsString();String roomName=roomInfo.get("name").getAsString();System.out.println((i+1)+". "+roomName+" (ID: "+roomId+")");} } else { System.out.println("\nFailed to list rooms: "+message); } break;
