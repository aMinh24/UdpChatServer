...
public class KickUserHandler implements CommandHandler {

    @Override
    public void handle(String args, ClientState clientState, HandshakeManager handshakeManager) {
        if (args == null || args.trim().isEmpty()) {
            System.out.println("Usage: " + Constants.CMD_KICK + " <room_id> <target_chatid>");
            System.out.print("> ");
            return;
        }
        ...
        if (clientState.getSessionKey() == null) {
            System.out.println("You must be logged in to kick a user. Use /login <id> <pw>");
            System.out.print("> ");
            return;
        }
        ...
        JsonObject request = JsonHelper.createRequest(Constants.ACTION_KICK_USER, data);
        handshakeManager.sendClientRequestWithAck(request, Constants.ACTION_KICK_USER, clientState.getSessionKey());
    }

    @Override
    public String getDescription() {
        return Constants.CMD_KICK + " <room_id> <target_chatid> - Kick a user from a room";
    }
}